        ...
        <td align="center">
            <div class="container">
                ...
                <table width="100%" cellpadding="0" cellspacing="0">
                    <tr>
                        <td class="header">
                            <h1 class="brand-name">{{ config('site.name', 'Septyaa Beauty Bar') }}</h1>
                ...
                <table width="100%" cellpadding="0" cellspacing="0">
                    <tr>
                        <td class="content">
                            <h2 class="greeting">{{ $data['subtitle'] }}</h2>
                            <p class="main-text">{!! \Illuminate\Support\Str::markdown($data['message'] ?? '') !!}</p>

                            <!-- METADATA TRANSAKSI -->
                            <table class="meta-table">
                                <tr>
                                    <td>No. Reservasi</td>
                                    <td class="meta-value">#{{ $booking->id }}</td>
                                </tr>
                                <tr>
                            ...
                            <table class="invoice-table">
                                ...
                                <tbody>
                                    <tr>
                                        <td>
                                            <span class="product-name">{{ $booking->service_name }}</span>
                                            <br><small style="color: #888;">Rp {{ number_format($booking->price ?? ($booking->service->price ?? 0), 0, ',', '.') }}</small>
                                        </td>
                                        <td class="text-right" style="color: #666;">1 Sesi</td>
                                        <td class="text-right font-weight-bold">Rp {{ number_format($booking->price ?? ($booking->service->price ?? 0), 0, ',', '.') }}</td>
                                    </tr>
                                    ...
                                    <tr class="total-row" style="border-top: 1px solid #1A1A1A;">
                                        ...
                                        <td class="text-right">Rp {{ number_format($booking->price ?? ($booking->service->price ?? 0), 0, ',', '.') }}</td>
                                    </tr>
                                    <tr class="total-row">
                                        <td colspan="1"></td>
                                        <td class="text-right grand-total">Total Akhir</td>
                                        <td class="text-right grand-total" style="color: #1A1A1A;">Rp {{ number_format($booking->price ?? ($booking->service->price ?? 0), 0, ',', '.') }}</td>
                                    </tr>
                                </tbody>
                            </table>
                            ...
                            <div class="action-container">
                                <a href="{{ $data['action_url'] }}" class="btn-luxury">
                                    {{ $data['action_text'] }}
                                </a>
                            </div>
                            @endif
                            ...
                            <p class="footer-note">{{ $data['footer_note'] }}</p>

                            <div class="signature">
                                Salam hangat,<br>
                                <strong>{{ config('site.name', 'Septyaa Beauty Bar') }}</strong>
                            </div>
                        </td>
                    </tr>
                </table>
