...
<div class="mb-8 grid grid-cols-2 gap-4 sm:grid-cols-4 sm:gap-6">
    <div class="rounded-2xl border border-line bg-surface p-5 text-center transition-all duration-300 hover:shadow-[0_8px_30px_rgb(0,0,0,0.02)]">
        ...
        <p class="mt-2 text-2xl font-bold tracking-tight text-ink">{{ str_pad($totalOrders, 2, '0', STR_PAD_LEFT) }}</p>
    </div>
    <div class="rounded-2xl border border-line bg-surface p-5 text-center transition-all duration-300 hover:shadow-[0_8px_30px_rgb(0,0,0,0.02)]">
        <p class="text-[11px] font-semibold uppercase tracking-wider text-ink-muted/80">Sudah Dibayar</p>
        <p class="mt-2 text-2xl font-bold tracking-tight text-emerald-600">{{ str_pad($paidOrders, 2, '0', STR_PAD_LEFT) }}</p>
    </div>
    <div class="rounded-2xl border border-line bg-surface p-5 text-center transition-all duration-300 hover:shadow-[0_8px_30px_rgb(0,0,0,0.02)]">
        <p class="text-[11px] font-semibold uppercase tracking-wider text-ink-muted/80">Menunggu</p>
        <p class="mt-2 text-2xl font-bold tracking-tight text-amber-500">{{ str_pad($pendingOrders, 2, '0', STR_PAD_LEFT) }}</p>
    </div>
    <div class="rounded-2xl border border-line bg-surface p-5 text-center transition-all duration-300 hover:shadow-[0_8px_30px_rgb(0,0,0,0.02)]">
        <p class="text-[11px] font-semibold uppercase tracking-wider text-ink-muted/80">Selesai</p>
        <p class="mt-2 text-2xl font-bold tracking-tight text-sky-600">{{ str_pad($completedOrders, 2, '0', STR_PAD_LEFT) }}</p>
    </div>
</div>

<!-- BAR ALAT: FITUR PENCARIAN DENGAN INTEGRASI FILTER KONSISTEN -->
<form method="GET" action="{{ route('admin.orders.index') }}" class="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center">
    ...
    <div class="relative flex-1">
        ...
        <input type="text" name="q" value="{{ request('q') }}" placeholder="Cari nomor order / nama pelanggan..."
...
<div class="mb-6 flex flex-wrap gap-2">
    <a href="{{ route('admin.orders.index', request()->only(['q'])) }}"
    ...
    @foreach ($statuses as $status)
        <a href="{{ route('admin.orders.index', array_merge(request()->only(['q']), ['status' => $status])) }}"
           class="rounded-xl px-4 py-2 text-xs font-semibold tracking-wide transition-all duration-200
           {{ $filterStatus === $status ? 'bg-accent text-white shadow-sm shadow-accent/20' : 'border border-line bg-surface text-ink hover:border-accent' }}">
            {{ \App\Models\Order::STATUS_LABELS[$status] ?? ucfirst($status) }}
        </a>
    @endforeach
</div>
                ...
                @forelse ($orders as $order)
                    <tr class="hover:bg-surface-light/30 transition-colors duration-150">
                        ...
                        <td class="px-6 py-4">
                            ...
                            <span class="block text-[10px] text-ink-muted/80 mt-0.5">{{ $order->created_at->format('d M Y &middot; H:i') }}</span>
                        </td>

                        <!-- Customer Details -->
                        <td class="px-6 py-4">
                            <span class="block font-medium text-ink text-xs tracking-tight">{{ $order->customer_name }}</span>
                            <span class="block font-mono text-[10px] text-ink-muted/80 mt-0.5">{{ $order->customer_phone }}</span>
                        </td>

                        <!-- Embedded Item Strings -->
                        <td class="px-6 py-4">
                            <span class="block text-xs text-ink-muted max-w-xs truncate font-medium" title="{{ $order->items->pluck('product_name')->implode(', ') }}">
                        ...
                        <td class="px-6 py-4 text-right">
                            <form method="POST" action="{{ route('admin.orders.destroy', $order) }}"
                                  onsubmit="return confirm('Hapus entri log order {{ $order->order_number }}?');">
                                @csrf
                                @method('DELETE')
