    ...
    <div class="flex min-h-screen">
                    ...
                    @foreach ($adminNav as $item)
                        ...
                        @if ($item['available'])
                            <a href="{{ route($item['route']) }}"
                                ...
                                {{ $isActive ? 'bg-accent text-white shadow-sm shadow-accent/20 font-semibold' : 'text-ink-muted hover:text-ink hover:bg-surface-light/70' }}">
                                <span class="{{ $isActive ? 'text-white' : 'text-ink-muted/70 group-hover:text-ink' }}">
                                    {!! $item['icon'] !!}
                                </span>
                                {{ $item['label'] }}
                            </a>
        ...
        <div class="flex-1 lg:pl-64 flex flex-col min-w-0">
            ...
            <header class="sticky top-0 z-10 flex h-16 items-center justify-between border-b border-line bg-surface/80 px-6 backdrop-blur-md sm:px-8">
                <div>
                    <h1 class="font-serif text-lg font-bold tracking-tight text-ink">@yield('title', 'Menu Utama')</h1>
                </div>
