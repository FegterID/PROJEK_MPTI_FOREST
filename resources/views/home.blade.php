...
<section class="relative overflow-hidden" data-hero-slider data-autoplay="5000">
    <div class="relative h-[560px] sm:h-[640px]">
        @foreach ($heroSlides as $index => $slide)
            <article data-slide
                class="absolute inset-0 bg-cover bg-center transition-opacity duration-700 {{ $index === 0 ? 'opacity-100' : 'opacity-0 pointer-events-none' }}"
                style="background-image: linear-gradient(180deg, rgba(20,14,10,0.15) 0%, rgba(20,14,10,0.55) 100%), url('{{ asset('images/stock/'.$slide['image']) }}');">
                ...
                <div class="mx-auto flex h-full max-w-6xl flex-col justify-center px-6">
                    ...
                    <h1 class="mt-3 max-w-xl font-serif text-4xl font-semibold text-white sm:text-6xl">{{ $slide['title'] }}</h1>
                    <p class="mt-4 text-sm text-white/80 sm:text-base">{{ $slide['subtitle'] }}</p>

                    <div class="mt-7 flex flex-wrap gap-3">
                        <a href="{{ $slide['primaryRoute'] }}"
        ...
        <div class="absolute bottom-6 left-1/2 flex -translate-x-1/2 gap-2">
            @foreach ($heroSlides as $index => $slide)
                <button type="button" data-slide-dot="{{ $index }}" aria-label="Ke slide {{ $index + 1 }}"
                        class="h-2.5 w-2.5 rounded-full transition {{ $index === 0 ? 'bg-white' : 'bg-white/40' }}"></button>
            @endforeach
        </div>
    </div>
</section>
    ...
    <div class="mt-10 grid gap-6 sm:grid-cols-2">
        <article class="group overflow-hidden rounded-2xl border border-line bg-surface">
            <div class="h-56 bg-cover bg-center transition duration-500 group-hover:scale-105"
                 style="background-image: linear-gradient(180deg, rgba(20,14,10,0.05) 0%, rgba(20,14,10,0.35) 100%), url('{{ asset('images/stock/service-nail.jpg') }}');"></div>
            <div class="p-6">
                ...
                <a href="{{ route('services.index') }}" class="mt-3 inline-block text-sm font-medium text-accent hover:underline">Jelajahi &rarr;</a>
            </div>
        </article>

        <article class="group overflow-hidden rounded-2xl border border-line bg-surface">
            <div class="h-56 bg-cover bg-center transition duration-500 group-hover:scale-105"
                 style="background-image: linear-gradient(180deg, rgba(20,14,10,0.05) 0%, rgba(20,14,10,0.35) 100%), url('{{ asset('images/stock/service-face.jpg') }}');"></div>
            <div class="p-6">
...
<section class="bg-surface py-20">
    <div class="mx-auto max-w-6xl px-6">
        <div class="flex items-end justify-between">
            ...
            <a href="{{ route('products.index') }}" class="hidden text-sm font-medium text-accent hover:underline sm:block">Jelajahi Produk &rarr;</a>
        </div>
        ...
        <div class="mt-10 grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
            @foreach ([
                ...
                <article class="overflow-hidden rounded-2xl border border-line bg-white">
                    <div class="h-40 bg-cover bg-center" style="background-image: url('{{ asset('images/stock/'.$item['image']) }}');"></div>
                    <div class="p-4">
                        <h4 class="font-serif text-sm font-semibold text-ink">{{ $item['name'] }}</h4>
                        <p class="mt-1 text-xs text-ink-muted">{{ $item['tag'] }}</p>
                    </div>
                </article>
            @endforeach
        </div>
...
<section class="mx-auto max-w-6xl px-6 py-20">
    <div class="grid gap-10 lg:grid-cols-2 lg:items-center">
        ...
        <div class="h-80 rounded-[34px] bg-cover bg-center shadow-lg lg:h-96"
             style="background-image: linear-gradient(180deg, rgba(255,248,240,0.1) 0%, rgba(92,62,42,0.25) 100%), url('{{ asset('images/stock/editorial.jpg') }}');"></div>
    </div>
</section>
