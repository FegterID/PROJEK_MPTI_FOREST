...
<div class="grid min-h-screen lg:grid-cols-2">
    <div class="relative hidden overflow-hidden bg-ink lg:flex lg:items-center lg:justify-center">
        ...
        <div class="relative z-10 max-w-xs px-10 text-center text-white">
            <p class="font-serif text-3xl font-medium">A Moment of Stillness</p>
            <p class="mt-3 text-sm text-white/70">
                Experience the art of curated self-care in our digital sanctuary.
            </p>
        </div>
    </div>
    ...
    <div class="relative flex items-center justify-center px-6 py-16">
        ...
        <div class="w-full max-w-sm">
            ...
            <h1 class="mt-2 font-serif text-3xl font-semibold text-ink">Join the Sanctuary</h1>
            ...
            <form method="POST" action="{{ route('register.attempt') }}" class="mt-6 space-y-5">
                @csrf
                <div>
                    <label for="name" class="block text-xs font-semibold uppercase tracking-wide text-ink-muted">Full Name</label>
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        <label for="phone" class="block text-xs font-semibold uppercase tracking-wide text-ink-muted">Phone Number</label>
                        <input type="tel" id="phone" name="phone" value="{{ old('phone') }}" required placeholder="0812xxxxxxxx"
                               class="mt-2 w-full rounded-lg border border-line bg-white px-4 py-2.5 text-sm text-ink focus:border-accent focus:outline-none focus:ring-2 focus:ring-accent/20">
                    </div>
                    <div>
                        <label for="email" class="block text-xs font-semibold uppercase tracking-wide text-ink-muted">Email</label>
                        <input type="email" id="email" name="email" value="{{ old('email') }}" required placeholder="hello@example.com"
                               class="mt-2 w-full rounded-lg border border-line bg-white px-4 py-2.5 text-sm text-ink focus:border-accent focus:outline-none focus:ring-2 focus:ring-accent/20">
                    </div>
                </div>

                <div>
                    <label for="address" class="block text-xs font-semibold uppercase tracking-wide text-ink-muted">Address</label>
                    <textarea id="address" name="address" rows="3" required placeholder="Alamat lengkap"
                              class="mt-2 w-full rounded-lg border border-line bg-white px-4 py-2.5 text-sm text-ink focus:border-accent focus:outline-none focus:ring-2 focus:ring-accent/20">{{ old('address') }}</textarea>
                </div>
                ...
                <p class="text-xs text-ink-muted">Your data will be used for booking and orders.</p>
            ...
            <p class="mt-6 text-center text-sm text-ink-muted">
                ...
                <a href="{{ route('login') }}" class="font-medium text-accent hover:underline">Login</a>
            </p>
        </div>
    </div>
</div>
