...
<div class="mb-8 grid grid-cols-3 gap-6">
    <div class="rounded-2xl border border-line bg-surface p-5 text-center transition-all duration-300 hover:shadow-[0_8px_30px_rgb(0,0,0,0.02)]">
        ...
        <p class="mt-2 text-2xl font-bold tracking-tight text-ink"><?php echo e(str_pad($totalPending, 2, '0', STR_PAD_LEFT)); ?></p>
    </div>
    <div class="rounded-2xl border border-line bg-surface p-5 text-center transition-all duration-300 hover:shadow-[0_8px_30px_rgb(0,0,0,0.02)]">
        <p class="text-[11px] font-semibold uppercase tracking-wider text-ink-muted/80">Selesai</p>
        <p class="mt-2 text-2xl font-bold tracking-tight text-ink"><?php echo e(str_pad($totalCompleted, 2, '0', STR_PAD_LEFT)); ?></p>
    </div>
    <div class="rounded-2xl border border-line bg-surface p-5 text-center transition-all duration-300 hover:shadow-[0_8px_30px_rgb(0,0,0,0.02)]">
        <p class="text-[11px] font-semibold uppercase tracking-wider text-ink-muted/80">Dibatalkan</p>
        <p class="mt-2 text-2xl font-bold tracking-tight text-ink"><?php echo e(str_pad($totalCancelled, 2, '0', STR_PAD_LEFT)); ?></p>
    </div>
</div>

<!-- BAR ALAT: BARU - FITUR PENCARIAN DENGAN INTEGRASI FILTER KONSISTEN -->
<form method="GET" action="<?php echo e(route('admin.bookings.index')); ?>" class="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center">
    ...
    <div class="relative flex-1">
        ...
        <input type="text" name="q" value="<?php echo e(request('q')); ?>" placeholder="Cari nama pelanggan atau nomor WhatsApp..."
...
<div class="mb-6 flex flex-wrap gap-2">
    <a href="<?php echo e(route('admin.bookings.index', request()->only(['q']))); ?>"
    ...
    <?php $__currentLoopData = $statuses; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $status): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
        <a href="<?php echo e(route('admin.bookings.index', array_merge(request()->only(['q']), ['status' => $status]))); ?>"
           class="rounded-xl px-4 py-2 text-xs font-semibold tracking-wide transition-all duration-200
                ...
                <?php $__empty_1 = true; $__currentLoopData = $bookings; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $booking): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                    <tr class="hover:bg-surface-light/30 transition-colors duration-150">
                        ...
                        <td class="px-6 py-4">
                            ...
                            <span class="block font-mono text-[10px] text-ink-muted/80 mt-0.5"><?php echo e($booking->whatsapp); ?></span>
                        </td>
                        ...
                        <!-- Jadwal -->
                        <td class="px-6 py-4 text-xs font-medium text-ink-muted">
                            <span class="block text-ink tracking-tight"><?php echo e($booking->booking_date->format('d M Y')); ?></span>
                            <span class="block text-[10px] text-ink-muted/80 mt-0.5">Pukul <?php echo e($booking->booking_time->format('H:i')); ?> WIB</span>
                        </td>
                        ...
                        <td class="px-6 py-4 text-right">
                            <form method="POST" action="<?php echo e(route('admin.bookings.destroy', $booking)); ?>"
                                  onsubmit="return confirm('Hapus entri booking #<?php echo e($booking->id); ?>?');">
                                <?php echo csrf_field(); ?>
